            ...
            for (int i = 0; i < header.FieldArray.Length; i++)
            {
                ...
                switch (header.FieldArray[i].DataType)
                {
                    case NativeDbType.Char:
                        if (!(values[i] is byte[]) && !(values[i] is String) && !(values[i] is DBNull))
                        {
            ...
            for (int j = 0; j < header.FieldArray.Length; j++)
            {
                ...
                switch (header.FieldArray[j].DataType)
                {
                    case NativeDbType.Char:
                        ...
                        {
                            if (objectArray[j] is byte[])
                            {
                                dataOutput.Write(Utils.textPadding((objectArray[j] as byte[]), header.FieldArray[j].FieldLength));
                            }
                            else
                            {
